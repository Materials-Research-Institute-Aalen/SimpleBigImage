...
namespace Cornerstones.Poly2DMath
{
    ...
    /// <summary>
    /// One of the external classes to run a polygon collision library
    /// </summary>
    ...
    public struct IntRect
    {
        ...
        public Path toPath()
        {
            ...
            output.Add(new IntPoint(this.left, this.top));
            output.Add(new IntPoint(this.right, this.top));
            output.Add(new IntPoint(this.right, this.bottom));
            output.Add(new IntPoint(this.left, this.bottom));
            output.Add(new IntPoint(this.left, this.top));
    ...
    public enum JoinType { jtSquare, jtRound, jtMiter, jtSquareH, jtSquareV };
    ...
    public class ClipperOffset
    {
        ...
        void OffsetPoint(int j, ref int k, JoinType jointype)
        {
            ...
            if (Math.Abs(m_sinA * m_delta) < 1.0)
            {
                ...
                {
                    if (jointype == JoinType.jtSquareH)
                    {
                        m_destPoly.Add(new IntPoint(Round(m_srcPoly[j].X + m_normals[k].X * m_delta), Round(m_srcPoly[j].Y)));
                        return;
                    }
                    if (jointype == JoinType.jtSquareV)
                    {
                        m_destPoly.Add(new IntPoint(Round(m_srcPoly[j].X), Round(m_srcPoly[j].Y + m_normals[k].Y * m_delta)));
                        return;
                    }
            ...
            if (m_sinA * m_delta < 0)
            {
                if (jointype == JoinType.jtSquareH)
                {
                    ...
                    m_destPoly.Add(new IntPoint(Round(m_srcPoly[j].X + m_normals[j].X * m_delta), Round(m_srcPoly[j].Y)));
                }
                else if (jointype == JoinType.jtSquareV)
                {
                    m_destPoly.Add(new IntPoint(Round(m_srcPoly[j].X), Round(m_srcPoly[j].Y + m_normals[k].Y * m_delta)));
                    m_destPoly.Add(m_srcPoly[j]);
                    m_destPoly.Add(new IntPoint(Round(m_srcPoly[j].X), Round(m_srcPoly[j].Y + m_normals[j].Y * m_delta)));
                }
                else
                {
                    m_destPoly.Add(new IntPoint(Round(m_srcPoly[j].X + m_normals[k].X * m_delta), Round(m_srcPoly[j].Y + m_normals[k].Y * m_delta)));
                ...
                switch (jointype)
                {
                    ...
                    case JoinType.jtSquareV: DoSquareV(j, k); break;
                    case JoinType.jtSquareH: DoSquareH(j, k); break;
        ...
        internal void DoSquareV(int j, int k)
        {
        ...
        internal void DoSquareH(int j, int k)
        {
            double dx = Math.Tan(Math.Atan2(m_sinA,
                m_normals[k].X * m_normals[j].X + m_normals[k].Y * m_normals[j].Y) / 4);
            m_destPoly.Add(new IntPoint(
